...
func LoadChain(certFiles []string) (*Certificate, []*x509.Certificate, error) {
	if len(certFiles) < 2 {
	...
	chain := make([]*x509.Certificate, len(certFiles)-1)
	...
	return &Certificate{Certificate: certs[0]}, chain, nil
}
